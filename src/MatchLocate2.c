 ...
 *      Copyright (c) 2013-2019 by M. Zhang
 *
...
int main(int argc, char **argv) {
    ...
    if(argc < 9 || error == 1) {
        ...
        fprintf(stderr, "       -T(window/before/after) -H(N*MAD) -D(INTD) -O(ouput) INPUT.in\n");
...
void DetermineEvent(EVENT *loc,long int NN) {
    ...
    fprintf(fp,"#Event   Time      Lat.      Lon.        Depth    Mag.    Coef.      N(*MAD)\n");
    ...
    if(NN > 0) {
        ...
        for(i=0; i<k; i++) {
            ...
            fprintf(fp,"%4d   %9.3lf   %7.4f   %8.4f   %6.2f   %5.2f    %6.4f    %6.4f \n",i+1,
                    loc2[i].eventTime,loc2[i].eventLat,loc2[i].eventLon,loc2[i].eventH,mag,loc2[i].eventCoef,loc2[i].eventNMAD);
            if(go == 1) {
